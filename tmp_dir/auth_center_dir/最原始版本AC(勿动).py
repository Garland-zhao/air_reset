...
from auth_center_dir.config import OIDC_CONFIG, KC_CONFIG
...
class OpenIDConnect:
    ...
    def __init__(self, app=None, credentials_store=None):
        ...
        self.cookie_serializer = TimedJSONWebSignatureSerializer(OIDC_CONFIG['SECRET_KEY'],
    ...
    def _process_callback(self):
        """Exchange auth code and save credentials into credentials_store

        Exchange the auth code for actual credentials,
        then redirect to the originally requested page.
        """
        print('func in _process_callback')
        try:
            state = self._json_loads(urlsafe_b64decode(request.args['state'].encode('utf-8')))
            code = request.args['code']
        except (KeyError, ValueError):
            logger.error("Can't retrieve state or auth code with request.args: {}".format(request.args),
                         exc_info=True)
            return True, self._oidc_error()

        # loads origin path
        try:
            origin_uri = self.extra_data_serializer.loads(state[self.DESTINATION])
        except BadSignature:
            logger.error('State field was invalid')
            return True, self._oidc_error()
        print('origin_uri =', origin_uri)
        print('code =', code)
        # make a request to keycloak.token_uri to exchange the auth code for OAuth credentials
        ...
        # set a persistent signed cookie containing the ID token with g
    ...
    def redirect_to_auth_server(self, origin_path):
        ...
        """
        state = {
            self.DESTINATION: self.extra_data_serializer.dumps(origin_path).decode('utf-8')
        }
        extra_params = {
    ...
    def _before_request(self):
        ...
        2 check oidc_token
        Sets g.oidc_id_token to the oidc_token if the user has successfully authenticated,
        else returns a redirect to login page so user can go try to authenticate.
        ...
        if not self.check_token(kc_token, request.url):
    ...
    def _after_request(self, response):
        ...
        if getattr(g, 'oidc_id_token_dirty', False):
            if g.oidc_id_token:
                slimed_token = self.slim_token(g.oidc_id_token, payload_items=self.BASE_TOKEN_ITEM)
    ...
    def check_token(self, kc_token, orgin_path, action='cached'):
        ...
        if self.check_action(kc_token):
            next_action = self.check_action_mapping.get(action, None)
            return self.check_token(kc_token, orgin_path, action=next_action) if next_action else True
        else:
            return False
    ...
    def _is_cached(self, kc_token):
        ...
        kc_token.access_token = OAuth2Credentials.from_json(cached_credential).access_token
    ...
    def _is_exp(self, kc_token):
        ...
        if time.time() < kc_token.payload['exp']:  # keycloak 默认 5min
            ...
            try:
                ...
                credentials = OAuth2Credentials.from_json(self.get_credentials(kc_token.payload))
            except KeyError:
            ...
            try:
                ...
                if credentials.id_token:
                    ...
                    # It is not guaranteed that we will get a new oidc_token on refresh,
                    # so if we do not, let's just update the id token expiry field
                    ...
                    if credentials.token_expiry is None:
                        ...
                        kc_token.payload['exp'] = time.time() + int(OIDC_CONFIG['Extended_Expiration'])
                    else:
                        kc_token.payload['exp'] = calendar.timegm(credentials.token_expiry.timetuple())
                ...
                logger.warning("Expired ID token, can't refresh credentials", exc_info=True)
                self.remove_credentials(kc_token.payload)
                return False
    ...
    def _is_scope(self, kc_token):
        ...
        check cards access
        """
        require_access_list = request.headers.get(self.ACCESS)
        if not require_access_list:
            logger.info('current page no need access')
            return True
        user_access = kc_token.payload.get(self.ACCESS, {})
        no_found_access = [access for access in require_access_list if access.strip() not in user_access]
        if no_found_access:
            logger.warning('current user has no access {}'.format(','.join(no_found_access)))
            return False
        return True
    ...
    def _set_kc_uris():
        ...
        """
        kc = dict()
        kc['issuer'] = urljoin(KC_CONFIG['KC_AUTH_URI'], '/auth/realms/{}'.format(KC_CONFIG['KC_REALMS']))
        kc['auth_uri'] = "/".join([kc['issuer'], 'protocol/openid-connect/auth'])
        kc['userinfo_uri'] = "/".join([kc['issuer'], 'protocol/openid-connect/userinfo'])
        kc['token_uri'] = "/".join([kc['issuer'], 'protocol/openid-connect/token'])
        kc['logout_uri'] = "/".join([kc['issuer'], 'protocol/openid-connect/logout'])
    ...
    def _flow_for_request(self):
        ...
        """
        flow = copy(self.flow)
        flow.redirect_uri = url_for(OIDC_CONFIG['CALLBACK_ENDPOINT'], _external=True)
        return flow
    ...
    def _is_id_token_valid(self, id_token):
        ...
        if not id_token.get('email_verified', False) and OIDC_CONFIG['OIDC_REQUIRE_VERIFIED_EMAIL']:
    ...
    def save_credentials(self, credentials, token):
        ...
        self.credentials_store[token_key] = credentials.to_json()
    ...
    def _set_cookie_id_token(self, id_token, changed=False):
        ...
        cooperates with @after_request to set a new id_token to cookie.
        """
    ...
    def _get_cookie_id_token(self):
        ...
        if cookie_token:
            try:
                ...
                logger.warning("ID token cookie Signature expired", exc_info=True)
                token_info = None
            except Exception:
                logger.error("Can not loads oidc_id_token :{}".format(cookie_token), exc_info=True)
    ...
    def get_credentials(self, token):
        ...
        """
        token_key = self.compute_token_key(token)
        return self.credentials_store.get(token_key, None)
    ...
    def remove_credentials(self, token):
        ...
        """
        token_key = self.compute_token_key(token)
        del self.credentials_store[token_key]
    ...
    def slim_token(self, id_token, payload_items=None):
        ...
        :rtype: dict
        """
        return {key: id_token[key] for key in payload_items if key in id_token} if payload_items else id_token
